...
async function loadBookData(version: string, bookAbbr: string): Promise<BibleBookData | null> {
    // ★ Vercel(Linux)은 대소문자를 구분하므로 항상 대문자로 변환
    const upperAbbr = bookAbbr.toUpperCase();
    const cacheKey = `${version}/${upperAbbr}`;

    if (bookDataCache.has(cacheKey)) {
        return bookDataCache.get(cacheKey)!;
    }

    const url = `/bible-data/${version}/${upperAbbr}.json`;
    try {
        const res = await fetch(url);
        if (!res.ok) {
            console.error(`[bible-data] ${url} 로드 실패 (HTTP ${res.status})`);
            return null;
        }
        ...
        console.error(`[bible-data] ${url} fetch 에러:`, err);
        return null;
    }
}
...
export function getBookInfo(bookAbbr: string): BookInfo | undefined {
    ...
    return BOOK_INFO.find((b) => b.abbr === upper);
}
